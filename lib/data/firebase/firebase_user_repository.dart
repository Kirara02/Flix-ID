...
class FirebaseUserRepository implements UserRepository {
  ...
  Future<Result<User>> getUser({required String uid}) async {
    DocumentReference<Map<String, dynamic>> documentRef = _firebaseFirestore.doc("users/$uid");

    DocumentSnapshot<Map<String, dynamic>> result = await documentRef.get();
    if (result.exists) {
      return Result.success(User.fromJson(result.data()!));
    } else {
  ...
  Future<Result<User>> updateUser({required User user}) async {
    try {
      DocumentReference<Map<String, dynamic>> docRef = _firebaseFirestore.doc('users/${user.uid}');
      ...
      if (result.exists) {
        User updateUser = User.fromJson(user.toJson());
        if (updateUser == user) {
